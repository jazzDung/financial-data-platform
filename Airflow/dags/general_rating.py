...
def data_to_csv():
    ...
    #Export to csv
    all_stock_general_rating.to_csv('/opt/airflow/data/General Rating.csv', sep=',', encoding='utf-8')


def csv_to_db():

    #Csv to dataframe
    data = pd.read_csv('/opt/airflow/data/General Rating.csv')
    ...
    data.to_sql('general_rating', con=conn, if_exists='replace', index=True)
    conn.close()


with DAG(
    dag_id='general_rating_v1',
    description='Import to General Rating table',
