...
def data_to_csv():
    ...
    #Export to csv
    all_stock_valuation_rating.to_csv('/opt/airflow/data/Valuation Rating.csv', sep=',', encoding='utf-8')


def csv_to_db():

    #Csv to dataframe
    data = pd.read_csv('/opt/airflow/data/Valuation Rating.csv')
    ...
    data.to_sql('valuation_rating', con=conn, if_exists='replace', index=True)
    conn.close()


with DAG(
    dag_id='valuation_rating_v3',
    description='Import to Valuation Rating table',
